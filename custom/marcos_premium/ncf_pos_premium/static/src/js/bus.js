odoo.define('ncf_pos_premium.pos_longpolling', function (require) {
        ...
        initialize: function (pos) {
            ...
            this.pos.longpolling_connection.on("change:poll_connection", function (status) {
                if (status) {
                    ...
                    if (!self.offline_sync_all_timer) {
                        var warning_message = "[1] No connection to the server. You can create new orders only. It is forbidden to modify existing orders.";
        ...
        send: function (message) {
            ...
            return send_it().fail(function (error, e) {
                ...
                if (error.message === 'XmlHttpRequestError ') {
                    ...
                    if (!self.offline_sync_all_timer) {
                        var warning_message = "[2] No connection to the server. You can create new orders only. It is forbidden to modify existing orders.";
                        self.warning(warning_message);
                        self.start_offline_sync_timer();
                    }
                } else {
                ...
                if (res.action == "revision_error") {
                    var warning_message = 'There is a conflict during synchronization, try your action again';
                    self.warning(warning_message);
                    self.request_sync_all();
                }
    ...
    exports.pos_sync_backend = Backbone.Model.extend({
        ...
        on_notification_do: function (channel, message) {
            if (Array.isArray(channel) && channel[1] === 'pos.sync.backend' && this.pos.config.sync_backend == true) {
                if (message.model == 'product.product') {
                    this.pos.update_product(message)
                }
                if (message.model == 'res.partner') {
                    this.pos.update_partner(message)
                }
            }
