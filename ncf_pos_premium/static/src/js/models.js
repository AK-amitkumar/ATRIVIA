...
odoo.define('ncf_pos_premium.models', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        initialize: function (session, attributes) {
            ...
            this.get('orders').bind('remove', function (order, collection, options) {
                if (!self.multi_session.client_online) {
                    if (order.order_on_server) {
                        var warning_message = "[3] No connection to the server. You can create new orders only. It is forbidden to modify existing orders.";
            ...
            this.pos_sync_backend = new exports.pos_sync_backend(this);
            var channel_name = "pos.sync.backend";
            var callback = this.sync_backend;
            this.add_channel(channel_name, callback, this);
        ...
        sync_backend: function (notification) {
            console.log("=================");
            console.log("sync_backend");
        ...
        ms_create_order: function (options) {
            var order = _.extend({pos: this}, options || {});
